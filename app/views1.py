...
def process(
    ...
    rec_joined[:, 3] = max(rec_joined[:, 3].astype('float')) / (
        rec_joined[:, 3].astype('float'))
    rec_joined[:, 11] = np.array(
        rec_joined[:, 11].astype('float') / min(rec_joined[:, 11].astype('float')))
    ...
    rec_max[0] = 1.0
    # combined rank
    r = np.sum((rec_joined[:, [0, 3, 11, 8, 7, 10]].astype('float') - rec_min) / (
        rec_max - rec_min) * rank_sel, 1) * rec_joined[:, 12].astype('float')
        ...
        if label not in pos_domains + neg_domains:
            ...
            graph["nodes"].append(
                {"id": str(node_id),
                 ...
                 "y": random.randint(-100,
                                     100),
                 "size": (max_size - min_size) * float(rec_col) + min_size,
                    "color": col})
            if i == 20:
                break
    ...
    for arc_id, source, target, weight_n2 in arcs:
        if str(source) in node_ids and str(target) in node_ids:
            graph[
                "edges"].append({"id": str(arc_id), "source": str(source), "target": str(target),
